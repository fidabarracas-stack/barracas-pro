...
async function loadVisitas() {
    ...
    try {
        const r=await fetch(`${API}/api/visitas`,{headers:authHeaders()});
        const v=await r.json();
        const l=document.getElementById("visitas-list");
        if(!v.length){l.innerHTML='<p style="color:#888;padding:30px;text-align:center;">Sin visitas</p>';return;}
        l.innerHTML=v.map(x=>`<div class="card"><div class="title">🏗️ ${x.barraca_nombre}</div><div class="subtitle">${x.fecha_planificada?x.fecha_planificada.substring(0,16).replace("T"," "):"Sin fecha"}</div><span class="estado estado-${x.estado}">${x.estado}</span><div class="actions">${x.estado==="planificada"?`<button class="btn btn-success btn-small" onclick="realizarVisita(${x.id},'compra')">✅ Compra</button><button class="btn btn-warning btn-small" onclick="realizarVisita(${x.id},'no_dinero')">💰 Sin dinero</button><button class="btn btn-danger btn-small" onclick="realizarVisita(${x.id},'no_interesa')">❌ No interesa</button>`:""}</div></div>`).join("");
    } catch(e){document.getElementById("visitas-list").innerHTML='<p style="color:red;">Error</p>';}
}

async function showVisitaModal(){
    const r=await fetch(`${API}/api/barracas`,{headers:authHeaders()});
    const b=await r.json();
    showModal(`<h3>Nueva Visita</h3><select id="vf-barraca"><option value="">Seleccionar...</option>${b.map(x=>`<option value="${x.id}">${x.nombre}</option>`).join("")}</select><input type="datetime-local" id="vf-fecha"><textarea id="vf-notas" placeholder="Notas..."></textarea><div class="actions"><button class="btn btn-primary" onclick="crearVisita()">Guardar</button><button class="btn btn-danger" onclick="hideModal()">Cancelar</button></div>`);
}

async function crearVisita(){
    const b=parseInt(document.getElementById("vf-barraca").value),f=document.getElementById("vf-fecha").value,n=document.getElementById("vf-notas").value;
    if(!b){alert("Seleccionar barraca");return;}
    await fetch(`${API}/api/visitas`,{method:"POST",headers:authHeaders(),body:JSON.stringify({barraca_id:b,fecha_planificada:f||null,notas:n})});
    hideModal();loadVisitas();
}

async function realizarVisita(id,res){
    await fetch(`${API}/api/visitas/${id}/realizar`,{method:"POST",headers:authHeaders(),body:JSON.stringify({resultado:res})});
    loadVisitas();
}
...
async function loadBarracas(){
    ...
    c.innerHTML=`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:15px;flex-wrap:wrap;gap:10px;"><h2>Gestion de Barracas</h2><div style="display:flex;gap:8px;"><button class="btn btn-warning" onclick="importarCafpadu()">🌐 Importar</button><button class="btn btn-success" onclick="geocodificarTodas()">📍 Geocodificar</button><button class="btn btn-primary" onclick="showBarracaModal()">+ Nueva</button></div></div><div id="import-status" style="display:none;margin-bottom:15px;padding:12px;background:#0f3460;border-radius:6px;font-size:0.9em;"></div><div id="geo-status" style="display:none;margin-bottom:15px;padding:12px;background:#0f3460;border-radius:6px;font-size:0.9em;"></div><div id="barracas-list"><p style="color:#888;">Cargando...</p></div>`;
    try{
        const r=await fetch(`${API}/api/barracas`,{headers:authHeaders()});
        const b=await r.json();
        const l=document.getElementById("barracas-list");
        if(!b.length){l.innerHTML='<p style="color:#888;padding:30px;text-align:center;">Sin barracas</p>';return;}
        l.innerHTML=b.map(x=>`<div class="card"><div class="title">🏗️ ${x.nombre}</div><div class="subtitle">${x.direccion||""} ${x.ciudad||""} ${x.departamento||""}<br>${x.telefono?'📞 '+x.telefono:''} ${x.web?'🌐 '+x.web.replace(/^https?:\/\//,''):''} ${x.facebook?'📘':''} ${x.instagram?'📷':''}</div><div class="actions"><button class="btn btn-primary btn-small" onclick="showBarracaModal(${x.id})">✏️ Editar</button><button class="btn btn-danger btn-small" onclick="deleteBarraca(${x.id})">🗑️</button></div></div>`).join("");
    }catch(e){document.getElementById("barracas-list").innerHTML='<p style="color:red;">Error</p>';}
}
...
async function importarCafpadu(){
    ...
    try{
        const r=await fetch(`${API}/admin/importar-cafpadu`,{method:"POST",headers:authHeaders()});
        const d=await r.json();
        e.innerHTML=r.ok?`✅ Encontradas: ${d.encontradas}, Guardadas: ${d.guardadas}, Duplicadas: ${d.duplicadas||0}`:`❌ ${d.detail||"Error"}`;
        loadBarracas();
...
async function showBarracaModal(id=null){
    ...
    <input type="text" id="b-facebook" placeholder="Facebook URL" value="${b.facebook||""}">
    <input type="text" id="b-instagram" placeholder="Instagram URL" value="${b.instagram||""}">
    <input type="text" id="b-twitter" placeholder="Twitter URL" value="${b.twitter||""}">
    <input type="text" id="b-whatsapp" placeholder="WhatsApp URL" value="${b.whatsapp||""}">
...
async function saveBarraca(id=null){
    const d={
        ...
        web:document.getElementById("b-web").value.trim()||null,
        facebook:document.getElementById("b-facebook").value.trim()||null,
        instagram:document.getElementById("b-instagram").value.trim()||null,
        twitter:document.getElementById("b-twitter").value.trim()||null,
        whatsapp:document.getElementById("b-whatsapp").value.trim()||null,
    ...
    try{
        if(id) await fetch(`${API}/api/barracas/${id}`,{method:"PUT",headers:authHeaders(),body:JSON.stringify(d)});
        else await fetch(`${API}/api/barracas`,{method:"POST",headers:authHeaders(),body:JSON.stringify(d)});
        hideModal();loadBarracas();
...
async function loadUsuarios(){
    const c=document.getElementById("v-usuarios");
    c.innerHTML=`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:15px;"><h2>Usuarios</h2><button class="btn btn-primary" onclick="showUserModal()">+ Nuevo</button></div><div id="usuarios-list"><p style="color:#888;">Cargando...</p></div>`;
    try{const r=await fetch(`${API}/admin/usuarios`,{headers:authHeaders()});const u=await r.json();document.getElementById("usuarios-list").innerHTML=u.map(x=>`<div class="card"><div class="title">${x.nombre}</div><div class="subtitle">@${x.username} - ${x.rol} ${!x.activo?"(inactivo)":""}</div></div>`).join("")||'<p style="color:#888;">Sin usuarios</p>';}catch(e){document.getElementById("usuarios-list").innerHTML='<p style="color:red;">Error</p>';}
}
...
async function createUser(){
    const d={username:document.getElementById("u-username").value.trim(),password:document.getElementById("u-password").value,nombre:document.getElementById("u-nombre").value.trim(),rol:document.getElementById("u-rol").value};
    if(!d.username||!d.password||!d.nombre){alert("Completar todos");return;}
    try{const r=await fetch(`${API}/admin/usuarios`,{method:"POST",headers:authHeaders(),body:JSON.stringify(d)});if(r.ok){hideModal();loadUsuarios();}else{const e=await r.json();alert(e.detail||"Error");}}catch(e){alert("Error");}
}
...
async function loadAsignaciones(){
    const c=document.getElementById("v-asignaciones");
    c.innerHTML=`<h2 style="margin-bottom:15px;">Asignar Barracas a Vendedores</h2><div style="display:flex;gap:10px;margin-bottom:20px;flex-wrap:wrap;"><select id="asig-vendedor" style="flex:1;padding:10px;background:#0f3460;color:white;border:1px solid #333;border-radius:6px;"></select><select id="asig-barraca" style="flex:1;padding:10px;background:#0f3460;color:white;border:1px solid #333;border-radius:6px;"></select><button class="btn btn-primary" onclick="asignar()">Asignar</button></div><div id="asig-list"><p style="color:#888;">Cargando...</p></div>`;
    try{
        const [ur,br,ar]=await Promise.all([fetch(`${API}/admin/usuarios`,{headers:authHeaders()}),fetch(`${API}/api/barracas`,{headers:authHeaders()}),fetch(`${API}/api/asignaciones`,{headers:authHeaders()})]);
        const [u,b,a]=await Promise.all([ur.json(),br.json(),ar.json()]);
        document.getElementById("asig-vendedor").innerHTML=u.filter(x=>x.rol==="vendedor"&&x.activo).map(x=>`<option value="${x.id}">${x.nombre}</option>`).join("");
        document.getElementById("asig-barraca").innerHTML=b.map(x=>`<option value="${x.id}">${x.nombre}</option>`).join("");
        document.getElementById("asig-list").innerHTML=a.map(x=>`<div class="card"><div class="title">${x.vendedor_nombre} ↔ ${x.barraca_nombre}</div><div class="actions"><button class="btn btn-danger btn-small" onclick="desasignar(${x.vendedor_id},${x.barraca_id})">Quitar</button></div></div>`).join("")||'<p style="color:#888;">Sin asignaciones</p>';
    }catch(e){console.error(e);}
}
...
function calcularMat(){
    const t=parseFloat(document.getElementById("c-techo")?.value)||0;
    const p=parseFloat(document.getElementById("c-pared")?.value)||0;
    const ct=parseFloat(document.getElementById("c-contra")?.value)||0;
    const v=parseFloat(document.getElementById("c-viga")?.value)||0;
    let r="";
...
async function init(){
    if(token){
        try{const r=await fetch(`${API}/api/barracas`,{headers:authHeaders()});if(r.ok)currentUser={username:"user",nombre:"Usuario",rol:"vendedor",id:0};}catch(e){token=null;localStorage.removeItem("bp_token");}
    }
    render();
}
